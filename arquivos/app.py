...
def get_desktop():
    # Essa função funciona em qualquer Windows (português ou inglês)
    CSIDL_DESKTOP = 0
...
PASTA_BASE = os.path.join(get_desktop(), 'SISTEMA_RODRIVIER')

PASTA_BANCOS = os.path.join(PASTA_BASE, 'bancos de dados')
PASTA_CARTAS_FRETE = os.path.join(PASTA_BASE, 'CARTAS-FRETE')
PASTA_IMAGENS = os.path.join(PASTA_BASE, 'imagens')

for pasta in [PASTA_BASE, PASTA_BANCOS, PASTA_CARTAS_FRETE, PASTA_IMAGENS]:
...
#BANCOS DE DADOS 
# MOTORISTAS
banco_mot = sqlite3.connect(os.path.join(PASTA_BANCOS,'MOTORISTAS.db'))
...
# PROPRIETARIOS
banco_prop = sqlite3.connect(os.path.join(PASTA_BANCOS,'PROPRIETARIOS.db'))
...
# VEICULOS
banco_vei = sqlite3.connect(os.path.join(PASTA_BANCOS,'VEICULOS.db'))
...
# CLIENTES
banco_cli = sqlite3.connect(os.path.join(PASTA_BANCOS,'CLIENTES.db'))
...
banco_recibo =  sqlite3.connect(os.path.join(PASTA_BANCOS,'NUMERO_RECIBO.db'))
...
def exibir_tabela_mot():
    ...
    if not container_nome_motorista.get() and not container_cpf_motorista.get():
        conexao = sqlite3.connect(os.path.join(PASTA_BANCOS,'MOTORISTAS.db'))
...
def exibir_tabela_prop():
    ...
    if not container_nome_proprietario.get() and not container_cpf_proprietario.get():
        conexao = sqlite3.connect(os.path.join(PASTA_BANCOS,'PROPRIETARIOS.db'))
        cursor = conexao.cursor()
...
def exibir_tabela_vei():
    ...
    if not container_renavan_veiculo.get() and not container_placa_veiculo.get():
        conexao = sqlite3.connect(os.path.join(PASTA_BANCOS,'VEICULOS.db'))
        cursor = conexao.cursor()
...
def exibir_tabela_cli():
    ...
    if not container_nome_cliente.get() and not container_cnpj_cliente.get():
        conexao = sqlite3.connect(os.path.join(PASTA_BANCOS,'CLIENTES.db'))
        cursor = conexao.cursor()
...
def emitir():
    ...
    try:
        ...
        planilha.save(os.path.join(PASTA_CARTAS_FRETE, nome_arquivo))
